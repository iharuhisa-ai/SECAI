...
interface StaffInput {
  ...
  // 構造化フィールド（隊員管理で設定。AIの配置判断に使う）
  available_shift_types?: string[] | null; // 対応可能な勤務区分（空/null=全区分可）
  fixed_off_weekdays?: number[] | null; // 固定休の曜日（0=日〜6=土）
  shift_lean?: string | null; // 勤務帯の優先（day / night / both）
  max_work_days?: number | null; // 月の勤務日数の上限
}
  ...
  properties: {
    shifts: {
      ...
      items: {
        ...
        properties: {
          day: { type: "INTEGER" },
          // 休は出力させない（出力に無い日は自動で休扱い）。明休は出力する。
          shift_type: { type: "STRING", enum: SHIFT_TYPES.filter((t) => t !== "休") },
          // 配置現場名。休・明休は空文字。
...
export async function POST(req: Request) {
  ...
  const staffLines = staff
    .map((s) => {
      ...
      // 構造化フィールド（優先度が高い＝確実に守る指示）
      if (s.available_shift_types && s.available_shift_types.length > 0)
        attrs.push(`対応可能区分:${s.available_shift_types.join("・")}`);
      if (s.fixed_off_weekdays && s.fixed_off_weekdays.length > 0)
        attrs.push(
          `固定休:${s.fixed_off_weekdays
            .slice()
            .sort((a, b) => a - b)
            .map((w) => WEEKDAY_LABELS[w])
            .join("・")}曜`
        );
      if (s.shift_lean === "day") attrs.push("勤務帯:日勤優先");
      else if (s.shift_lean === "night") attrs.push("勤務帯:夜勤優先");
      if (typeof s.max_work_days === "number") attrs.push(`勤務日数上限:${s.max_work_days}日`);
      // 自由記述（補助）
...
勤務区分は次のみ使用します: ${SHIFT_TYPES.join(" / ")}
- 日勤: 昼間の勤務
- 夜勤: 夜間の勤務
- 受付: 受付業務
- 半日: 短時間勤務
...
必ず守るルール:
1. **出力するのは「勤務日（日勤/夜勤/受付/半日/SV）」と「明休」だけ**。休の日は出力しない（出力に含めなかった日は自動的に「休」として扱われる）。これにより出力を軽くする。
2. 「確定済み(変更不可)」の日は出力に含めない。
3. 夜勤の翌日は必ず「明休」を出力する（月末日が夜勤の場合は明休は不要）。
4. 連続勤務は確定済みも通算して最大5日まで。それを超える前に休（＝その日を出力しない）を入れる。
5. 各隊員に週あたり1〜2日程度の休（＝出力しない日）を確保し、負担が偏らないようにする。
6. 月内で計画する日数（勤務＋明休＋休）は必ず1〜${daysInMonth}日の範囲。休は出力しないので、出力は勤務日と明休のみになる。
...
- **その日の不足枠がすべて0（充足済み）なら、無理に勤務させずその日は出力しない（＝休）**（過剰配置を避け、週休・連続勤務のバランスを取る）。
...
隊員ごとの設定の扱い（構造化フィールドは厳守）:
- **「対応可能区分」がある隊員は、その区分以外の勤務を絶対に割り当てない**（例: 対応可能区分が「日勤・受付」なら夜勤を入れない。「受付」を含む隊員だけが受付に入れる）。記載が無い隊員は全区分に対応可。
- **「固定休」の曜日は原則「休」にする**（祝日は日曜扱い）。
- **「勤務帯:日勤優先」は日勤中心、「勤務帯:夜勤優先」は夜勤中心**に配置する（対応可能区分の範囲内で）。
- **「勤務日数上限」がある隊員は、その月の勤務日数（休・明休を除く）が上限を超えないようにする**。
- 「休日希望」（自由記述）はできる限り尊重する。
...
「確定済み(変更不可)」と「まだ不足している枠」を踏まえ、空いている日のシフト（勤務区分＋配置現場＋時間帯）を作成してください。不足している枠を埋めることを最優先しつつ、この隊員の勤務シフト数が目安（約${targetWorkShifts}回）に近くなるようにし、その日の枠が充足済み、または目安に達したら休にしてください。特に希望のない隊員は日勤と夜勤の回数を均等にしてください。`;
  ...
  try {
    ...
    // 出力に無い日は「休」で補完する（AIは勤務日と明休のみ出力）。
    // 確定済み（既存入力）の日は除外して上書きしない。
    const usedDays = new Set<number>(shifts.map((s) => s.day));
    for (const e of existing) if (e.staff_id === targetStaffId) usedDays.add(e.day);
    for (let d = 1; d <= daysInMonth; d++) {
